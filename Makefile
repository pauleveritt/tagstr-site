...
build-playground-without-venv: check-jq
	cd playground  && jupyter lite build && \
		jq '.["jupyter-config-data"]["litePluginSettings"]["@jupyterlite/pyodide-kernel-extension:kernel"].pyodideUrl = "./pyodide.js"' dist/jupyter-lite.json > temp.json && mv temp.json dist/jupyter-lite.json
